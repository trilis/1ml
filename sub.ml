...
let rec sub_typ oneway env t1 t2 ps =
  ...
  let ts, zs, e =
    ...
    | StrT(tr1), StrT(tr2) ->
      let ts, zs, fs = sub_row oneway env tr1 tr2 ps in
      ts, zs,
    ...
    | FunT(aks1, t11, s1, Explicit p1), FunT(aks2, t21, s2, Explicit p2) ->
      ...
      let ts2, zs2, f2 =
        try sub_extyp oneway env' (subst_extyp (subst aks1 ts1) s1) s2 ps'
    ...
    | InferT(z) as t1, (TypT(_) as t2) ->
      ...
      let ts, zs2, f = sub_typ oneway env t1' t2 ps in
    ...
    | InferT(z) as t1, (FunT([], t21, ExT([], t22), Explicit Impure) as t2) ->
      ...
      let ts, zs3, f = sub_typ oneway env t1' t2 ps in
      if not (resolve_typ z t1') then raise (Sub (Mismatch(t1, t2)));
...
and sub_row oneway env tr1 tr2 ps =
  ...
  | (l, t2)::tr2' ->
    ...
    let ts2, zs2, fs = sub_row oneway env tr1 (subst_row su tr2') ps' in
